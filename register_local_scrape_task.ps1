param(
    [string]$TaskName = 'ObitScraper-LocalScrapeToNeon',
    [string]$Time = '07:00'
)
...
$runnerPath = Join-Path $projectRoot 'scheduled_local_scrape_runner.ps1'
...
if ($null -ne $existingTask) {
    ...
    Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger -Settings $settings -Description 'Run obituary scrapers locally and ingest results into Neon Postgres.' | Out-Null
}
